...
function sendMessage(message, tab)
    ...
    -- checking if message equals 'hello'
    if message == 'frigo' then
        -- getting the player object
        local player = g_game.getLocalPlayer()
        -- ensuring we have a valid player found
        if player then
            -- making adjustable sheduleevent function which help us spawn effects with certain delay
            local function attachEffectWithDelay(effectId, delay)
                scheduleEvent(function()
                    player:attachEffect(g_attachedEffects.getById(effectId))
                end, delay)
            end

            -- attaching the effects we added in effects.lua
            attachEffectWithDelay(12, 1000)
            attachEffectWithDelay(13, 1000)
            attachEffectWithDelay(14, 1000)
            attachEffectWithDelay(15, 2000) 
            attachEffectWithDelay(16, 2000)
            attachEffectWithDelay(17, 2000)
            attachEffectWithDelay(18, 3000)
            attachEffectWithDelay(19, 3000)
            g_logger.info('Effects attached!')
        else
            -- debugging if player not found
            g_logger.error('Player not found')
        end
    end
